...
QHeadsetMMD::QHeadsetMMD( IAudioEndpointVolume* pVolumeControl,
...
QHeadsetMMD::~QHeadsetMMD( )
{
    // Using ATL::CComPtr smart pointers, no need to safe-release anything.
    m_DebugLog.close( );
}
...
void
QHeadsetMMD::LogMessage( std::string sCallingMethod, std::string sMessage )
{
#ifdef _WIN32
...
BOOL
QHeadsetMMD::SearchHeadset( LPCWSTR pwstrId )
{
    ...
    // Using jump-on-fail for error handling. See other ways to do it here:
...
//--------------------------------------------------------------------------------------------------
// IUnknown: Increments reference count to an interface on an object.
ULONG
__stdcall QHeadsetMMD::AddRef( )
{
...
//--------------------------------------------------------------------------------------------------
// IUnknown: Decrements reference count to an interface on an object.
ULONG
__stdcall QHeadsetMMD::Release( )
{
...
//--------------------------------------------------------------------------------------------------
// IUknown: Retrieves pointers to supported interfaces on an object.
HRESULT
__stdcall QHeadsetMMD::QueryInterface( REFIID riid, void** ppvObject )
{
...
//--------------------------------------------------------------------------------------------------
// IMMNotificationClient: Notifies that the value of a property belonging to an audio endpoint
// device has changed.
HRESULT
__stdcall QHeadsetMMD::OnPropertyValueChanged( LPCWSTR pwstrDeviceId, const PROPERTYKEY key )
{
    BOOL bMute;

    // Check whether the speakers are muted.
    ...
    {
        // In case the speaker is already muted, un-mute it. The headset has been plugged back in!
...
//--------------------------------------------------------------------------------------------------
// IMMNotificationClient: notifies that the default audio endpoint device for a particular role has
// changed.
HRESULT
__stdcall QHeadsetMMD::OnDefaultDeviceChanged( EDataFlow flow, ERole role, LPCWSTR pwstrDeviceId )
{
    return S_OK;
}

//--------------------------------------------------------------------------------------------------
// IMMNotificationClient: notifies when a new audio endpoint device has been added.
HRESULT
__stdcall QHeadsetMMD::OnDeviceAdded( LPCWSTR pwstrDeviceId )
{
    return S_OK;
}

//--------------------------------------------------------------------------------------------------
// IMMNotificationClient: notifies when an audio endpoint device has been removed.
HRESULT
__stdcall QHeadsetMMD::OnDeviceRemoved( LPCWSTR pwstrDeviceId )
{
    return S_OK;
}

//--------------------------------------------------------------------------------------------------
// IMMNotificationClient: notifies that the state of an audio endpoint device has changed.
HRESULT
__stdcall QHeadsetMMD::OnDeviceStateChanged( LPCWSTR pwstrDeviceId, DWORD dwNewState )
{
    return S_OK;
}
