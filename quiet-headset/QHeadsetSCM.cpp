...
QHeadsetSCM::QHeadsetSCM( )
{
    // Structure containing data about the service's status
    ...
    // An error code the service uses to report an error that occurs while it's starting or
    // stopping
    m_ServiceStatus.dwWin32ExitCode = 0;

    // An error code service-specific that reports an error that occurs while it's starting or
    // stopping
...
void
QHeadsetSCM::UpdateServiceStatus( DWORD pControlsAccepted,
                                  DWORD pCurrentState,
                                  DWORD pWin32ExitCode,
                                  DWORD pCheckPoint )
{
 ...
 * It initialises anything we need, registers the service control handler which will handle the
 * service's different status (STOP, PAUSE, CONTINUE, ...)
 * It also serves to set the service status and perform startup tasks such as creating mutex,
 * threads, IPCs and such.
 */
VOID WINAPI
QHeadsetSCM::ServiceMain( DWORD argc, LPTSTR* argv )
{
    DWORD Status = E_FAIL;

    // Registers the service control handler with the SCM.
    getInstance( )->m_ServiceStatusHandle
        = RegisterServiceCtrlHandler( SERVICE_NAME,  // Service Name
                                      []( DWORD ctrlCode )
                                      {
                                          return getInstance( )->ServiceCtrlHandler( ctrlCode );
                                      }  // Service Handler: Fixed C3867 with lambda
                                      );
    ...
    // Updates the SCM's status information for the calling service, in other words we start the
    // service control here:
    if ( SetServiceStatus( getInstance( )->m_ServiceStatusHandle,
                           &( getInstance( )->m_ServiceStatus ) )
         == FALSE )
    {
        OutputDebugString( SERVICE_ERROR );
    }
    ...
    getInstance( )->m_ServiceStopEvent = CreateEvent(
        NULL,   // lpEventAttributes: The handle cannot be inherited by child processes.
        FALSE,  // bManualReset: Creates a manual-reset event object.  ResetEvent sets event state
                // to nonsignaled.
    ...
    HANDLE hThread = CreateThread( NULL, 0,
                                   []( LPVOID lpParam )
                                   {
                                       return getInstance( )->ServiceWorkThread( lpParam );
                                   },  // function executed by the thread: Fixed C3867 with lambda
                                   NULL,
                                   0, NULL );
...
VOID WINAPI
QHeadsetSCM::ServiceCtrlHandler( DWORD ctrlCode )
{
    switch ( ctrlCode )
    {
...
DWORD WINAPI
QHeadsetSCM::ServiceWorkThread( LPVOID lpParam )
{
...
HRESULT
QHeadsetSCM::ServiceMMDNotification( )
{
    HRESULT hr = S_FALSE;
    ...
    // Initialise the COM library on a thread.
    CoInitialize( NULL );
    ...
    if ( SUCCEEDED( hr ) )
    {
        // We look first for the default audio endpoint (speakers) which will have their volume
        // lowered/muted
        ...
        // We create a COM object with the IAudioEndpointVolume interface
        hr = d_Speaker->Activate(
        ...
        if ( SUCCEEDED( hr ) )
        {
            ...
            hr = d_Enumerator->RegisterEndpointNotificationCallback(
                static_cast< IMMNotificationClient* >( m_MultimediaInstance ) );
        }
    }
